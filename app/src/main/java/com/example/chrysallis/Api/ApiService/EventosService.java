...
public interface EventosService {
    ...
    Call<ArrayList<Evento>> busquedaEventosComunidadDate(@Path("id_comunidad")int id_comunidad,@Path("date")Date date);

    @GET("api/Eventos/Search/{nombre}/{id_comunidad}/{date}")
    Call<ArrayList<Evento>> busquedaEventosAll(@Path("nombre")String nombre,@Path("id_comunidad")int id_comunidad,@Path("date")Date date);
}
